...
const ChatSupport: React.FC<ChatSupportProps> = ({ onClose, initialChatId, userRole = 'student' }) => {
  ...
  useEffect(() => {
    ...
    socket.on('new_message', (data: { chatId: string; message: any; notification: any }) => {
  ...
  const handleSendMessage = async () => {
    ...
    if (socket && isConnected) {
      ...
      const newMessage = {
        sender: userRole as 'student' | 'admin',
        message: chatInput,
        timestamp: new Date().toISOString()
      };
            ...
            {showNewChatForm ? (
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <textarea
                        ...
                        placeholder={userRole === 'admin' ? 'Type your response...' : 'Type your message...'}
                        rows={1}
                        style={{
                          width: '100%',
                          padding: '12px 16px',
              ...
              <div style={{
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 14 }}>
                    {userRole === 'admin' ? 'Choose a conversation from the list to respond' : 'Choose a conversation from the list or start a new one'}
                  </div>
                </div>
              </div>
            )}
